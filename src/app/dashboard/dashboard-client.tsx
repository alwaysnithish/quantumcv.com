...
export default function DashboardClient({
  ...
  // stats/list are server-rendered props. Next.js can hand back a cached
  // copy of this page when navigating back from /builder (e.g. right after
  // a resume finishes generating), so the "Generated" count can look stuck.
  // Forcing a refresh whenever the dashboard mounts or regains focus pulls
  // the latest numbers from the DB without a full page reload.
  useEffect(() => {
    router.refresh();
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between gap-2 mt-3.5">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/resumeanalyser?resume=${r.id}`);
                      }}
                      className="inline-flex items-center gap-1 rounded-full border border-[var(--border)] px-2.5 py-1 text-[11px] font-semibold text-[var(--fg-muted)] hover:border-[var(--accent)]/50 hover:text-[var(--accent)] hover:bg-[var(--accent)]/5 transition-colors"
                    >
                      <ScanSearch size={12} /> Analyse
                    </button>
                  </div>
                </div>
              </div>
