...
// ─────────────────────────────────────────────────────────
// SET 3 — 9 new templates, added on top of the original 30.
// Each of these has a layout mechanic the 30 above didn't already
// cover (per-entry date/location column, auto-generated stat strip,
// alternating tinted panels, numbered wayfinding rail, initials badge,
// auto-bolded metrics, drop-cap summary, flattened 4-col skills matrix).
// A few other candidates were deliberately left out because they
// duplicated a mechanic already in SET_1/SET_2 (e.g. dot-rail timeline,
// Europass table, academic double-rule, skills-first reorder,
// compact-dense, monospace dev theme, gradient banner, dark/colored
// true sidebar) — no point shipping a near-copy of what's already here.
// ─────────────────────────────────────────────────────────
...
function gridEntryHtml(en: AnyData, ac: string, secId: string): string {
  ...
  return `<div style="display:grid;grid-template-columns:106px 1fr;column-gap:16px;margin-bottom:14px">
    <div style="font-size:0.65rem;color:#64748b;font-family:monospace;line-height:1.6;padding-top:2px">
      <div contenteditable="true" spellcheck="false" style="outline:none;border-radius:2px;cursor:text"
    ...
    <div style="min-width:0">
      <div contenteditable="true" spellcheck="false"
        style="font-size:0.82rem;font-weight:700;color:#0f172a;outline:none;border-radius:2px;cursor:text;word-break:break-word"
        oninput="typeof handleEdit!=='undefined'&&handleEdit('title','${secId}','${en.id}',-1,this.textContent)"
      >${esc(en.title)}</div>
      ${
        en.subtitle
          ? `<div contenteditable="true" spellcheck="false"
        style="font-size:0.75rem;font-style:italic;color:#475569;margin:1px 0 3px;outline:none;border-radius:2px;cursor:text"
      ...
      <div style="display:flex;gap:5px;margin-bottom:2px;line-height:1.5;align-items:flex-start;position:relative"
        ...
        <span contenteditable="true" spellcheck="false"
          style="flex:1;font-size:0.78rem;color:#334155;outline:none;border-radius:2px;word-break:break-word;cursor:text;line-height:1.55"
...
function metricEntryHtml(en: AnyData, ac: string, secId: string): string {
  ...
  return `<div style="margin-bottom:12px">
    <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;min-width:0">
      <div contenteditable="true" spellcheck="false"
        style="font-size:0.82rem;font-weight:700;color:#0f172a;flex:1;min-width:0;outline:none;border-radius:2px;word-break:break-word;cursor:text"
        oninput="typeof handleEdit!=='undefined'&&handleEdit('title','${secId}','${en.id}',-1,this.textContent)"
      >${esc(en.title)}</div>
      <div contenteditable="true" spellcheck="false"
        style="font-size:0.69rem;color:#64748b;white-space:nowrap;flex-shrink:0;font-family:monospace;outline:none;border-radius:2px;cursor:text"
        oninput="typeof handleEdit!=='undefined'&&handleEdit('date','${secId}','${en.id}',-1,this.textContent)"
      >${esc(dateStr)}</div>
    </div>
    ${
...
const SET_3: Omit<ResumeTemplate, 'premium'>[] = [
  {
    ...
    render(data, ac) {
      const secs = [...(data.sections || [])].sort((a: AnyData, b: AnyData) => (a.order || 0) - (b.order || 0));
      // Optional, off unless the person has actually filled these in — see
      // data.personal_info.{dob,marital_status,nationality,sex}. Discouraged
      // for US/UK/CA/AU resumes; only include for regions/users where it's
      // customary (e.g. some EU countries).
      ...
      return `
        <div style="text-align:center;margin-bottom:16px">
          <div style="font-size:1.35rem;font-weight:700;color:#0f172a">${editableName(data)}</div>
    ...
    render(data, ac) {
      ...
      return `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;margin-bottom:16px">
          <div style="min-width:0">
            <div style="font-size:1.35rem;font-weight:800;color:${ac};letter-spacing:0.01em">${editableName(data)}</div>
  ...
  },
  {
    ...
    render(data, ac) {
      ...
      return `
        ...
        <div style="font-size:0.68rem;color:#94a3b8;margin-bottom:18px;padding-bottom:14px;border-bottom:1px solid #e2e8f0">${contacts(data)}</div>
        ${allSections(data, ac)}`;
    },
  },
  {
    ...
    render(data, ac) {
      return `
        <div style="margin-bottom:18px;padding-bottom:14px;border-bottom:2px solid ${ac}">
          <div style="font-size:1.4rem;font-weight:800;color:#0f172a;letter-spacing:-0.02em">${editableName(data)}</div>
  ...
  },
  {
    ...
    render(data, ac) {
      return `
        <div style="text-align:center;padding-bottom:16px;margin-bottom:18px">
          ...
          <div style="border-top:3px solid #0f172a;border-bottom:1px solid #0f172a;padding:10px 0">
            <div style="font-family:Georgia,'Times New Roman',serif;font-size:1.9rem;font-weight:700;letter-spacing:0.01em;color:#0f172a">${editableName(data)}</div>
          </div>
  ...
  },
  {
    ...
    render(data, ac) {
      ...
      return `
        <div style="margin-bottom:16px;padding-bottom:14px;border-bottom:2px solid #0f172a">
          <div style="font-size:1.4rem;font-weight:800;color:#0f172a;letter-spacing:-0.02em">${editableName(data)}</div>
          <div style="font-size:0.75rem;color:${ac};font-weight:700;margin-top:3px">${editableRole(data, ac)}</div>
          <div style="font-size:0.68rem;color:#64748b;margin-top:6px">${contacts(data)}</div>
        </div>
        ${
          cards.length
            ? `<div style="display:grid;grid-template-columns:repeat(${cards.length},1fr);gap:10px;margin-bottom:20px">
          ${cards
            .map(
              (c) => `
  ...
  },
  {
    ...
    render(data, ac) {
      const secs = [...(data.sections || [])].sort((a: AnyData, b: AnyData) => (a.order || 0) - (b.order || 0));
      return `
        <div style="margin-bottom:20px;padding-left:38px">
          <div style="font-size:1.4rem;font-weight:800;color:#0f172a;letter-spacing:-0.02em">${editableName(data)}</div>
          <div style="font-size:0.75rem;color:${ac};font-weight:700;margin-top:3px">${editableRole(data, ac)}</div>
          <div style="font-size:0.68rem;color:#64748b;margin-top:6px">${contacts(data)}</div>
        </div>
  ...
  },
  {
    ...
    render(data, ac) {
      return `
        <div style="margin-bottom:20px">
          <div style="font-size:1.7rem;font-weight:300;color:#0f172a;letter-spacing:0.02em">${editableName(data)}</div>
...
// The original 7 free templates, unchanged. The 9 SET_3 templates are
// premium by default (see FREE_TEMPLATE_IDS) — add any of their ids here
// if you want to give them away free too.
...
export const TEMPLATES: ResumeTemplate[] = [...SET_1, ...SET_2, ...SET_3].map((t) => ({
  ...t,
