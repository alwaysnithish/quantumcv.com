...
/**
 * Escapes a value like esc(), but re-opens a narrow safelist of inline
 * formatting tags produced by the selection toolbar: <b>, <mark style="...">,
 * <span style="font-size:...">, <span style="font-family:...">. The style
 * attribute is restricted to a plain char set (no parens/quotes/colons-only-
 * for-known-props), so this can't be used to inject scripts or CSS
 * expressions even though the field is stored as a plain string.
 */
const RICH_TAG_RE = /&lt;(\/?)(b|mark|span)((?:\s+style=&quot;[a-zA-Z0-9#:;.,%\- ']*&quot;)?)&gt;/g;
export function escRich(s: unknown): string {
  if (s == null) return '';
  return esc(s).replace(RICH_TAG_RE, (_m, slash, tag, styleAttr) => {
    const style = styleAttr ? styleAttr.replaceAll('&quot;', '"') : '';
    return `<${slash}${tag}${style}>`;
  });
}
...
/** Round "disc" bullet marker — replaces the old ▸ glyph for a crisper, print-safe dot */
export function bulletDot(ac: string, size = 'calc(4.5px * var(--rs-font, 1))'): string {
  return `<span contenteditable="false" style="display:inline-block;width:${size};height:${size};
    border-radius:50%;background:${ac};flex-shrink:0;margin-right:6px;
    transform:translateY(calc(0.62em - ${size} / 2))"></span>`;
}
...
export function secHead(title: string, ac: string, secId?: string): string {
  const handler = secId
    ...
    : '';
  return `<div class="resume-section-heading" contenteditable="true" spellcheck="false" ${handler}
    style="font-size:calc(0.67rem * var(--rs-font, 1));font-weight:800;text-transform:uppercase;letter-spacing:0.14em;
      color:${ac};border-bottom:1.5px solid ${ac}33;padding-bottom:3px;
      margin-bottom:calc(8px * var(--rs-gap, 1));
...
/** Single entry (experience / education / project / etc.) — print-safe bullet rows */
export function entryHtml(en: AnyData, ac: string, secId: string, compact = false): string {
  ...
  const mb = compact ? 5 : 8;

  return `<div class="resume-entry" data-entry-id="${en.id}" style="margin-bottom:calc(${mb}px * var(--rs-gap, 1))">
    <div style="display:flex;justify-content:space-between;align-items:baseline;gap:10px;flex-wrap:wrap">
      <div style="font-size:calc(${fs}rem * var(--rs-font, 1));font-weight:700;color:#0f172a;word-break:break-word;flex:1;min-width:0"
      ><span data-edit-field="title"
        style="border-radius:2px;cursor:text"
      >${escRich(en.title)}</span>${
        en.subtitle
          ? ` <span style="font-weight:600;color:${ac}">[<span data-edit-field="subtitle"
        style="border-radius:2px;cursor:text">${escRich(en.subtitle)}</span>]</span>`
          : ''
      }</div>
      <div style="text-align:right;flex-shrink:0;white-space:nowrap">
        <div data-edit-field="date"
          style="font-weight:400;font-size:calc(0.69rem * var(--rs-font, 1));color:#64748b;font-family:monospace;border-radius:2px;cursor:text"
        >${esc(dateStr)}</div>
      </div>
    </div>
    ${
      ...
      ${en.bullets
        ...
        <div class="resume-bullet" style="position:relative;display:flex;align-items:flex-start;
            margin-bottom:calc(2px * var(--rs-gap, 1));line-height:calc(1.32 * var(--rs-line, 1))"
          ...
          ${bulletDot(ac)}<span data-edit-field="bullet" data-bullet-index="${bi}"
            style="display:inline;font-size:calc(${bfs}rem * var(--rs-font, 1));color:#334155;border-radius:2px;
              word-break:break-word;cursor:text;line-height:calc(1.32 * var(--rs-line, 1));flex:1;min-width:0"
          >${escRich(b)}</span>
          <div class="bullet-actions" contenteditable="false" style="display:none;gap:3px;align-items:center;position:absolute;right:0;top:0">
...
export function skillGroupsHtml(sec: AnyData, ac: string): string {
  return (sec.skill_groups || [])
    .map(
      (g: AnyData) => `
    <div style="display:flex;gap:8px;margin-bottom:calc(5px * var(--rs-gap, 1));align-items:flex-start;min-width:0">
      <div contenteditable="true" spellcheck="false"
        style="font-size:calc(0.71rem * var(--rs-font, 1));font-weight:700;color:#0f172a;min-width:70px;max-width:90px;
      ...
      >${esc(g.category)}</div>
      <div contenteditable="true" spellcheck="false"
        style="font-size:calc(0.73rem * var(--rs-font, 1));color:#334155;line-height:calc(1.4 * var(--rs-line, 1));flex:1;min-width:0;
...
export function summaryHtml(sec: AnyData): string {
  return `<p contenteditable="true" spellcheck="false"
    style="font-size:calc(0.8rem * var(--rs-font, 1));color:#334155;line-height:calc(1.4 * var(--rs-line, 1));margin:0;
...
export function langHtml(sec: AnyData): string {
  return (sec.entries || [])
    .map(
      (en: AnyData) => `
    <div style="font-size:calc(0.77rem * var(--rs-font, 1));margin-bottom:calc(3px * var(--rs-gap, 1))">
      <span style="font-weight:600">${esc(en.title)}</span><span style="color:#64748b">&nbsp;·&nbsp;${esc(en.subtitle || '')}</span>
    </div>`
    )
    .join('');
}
...
export function skillBarsHtml(sec: AnyData, ac: string): string {
  ...
  return `<div style="display:flex;flex-direction:column;gap:calc(7px * var(--rs-gap, 1))">
    ${(sec.skills || [])
      .map(
        (s: AnyData, i: number) => `
...
export function skillDotsHtml(sec: AnyData, ac: string): string {
  const n = (sec.skills || []).length;
  return `<div style="display:flex;flex-direction:column;gap:calc(6px * var(--rs-gap, 1))">
    ${(sec.skills || [])
      .map(
        (s: AnyData, i: number) => `
    ...
    ${(sec.tags || [])
      ...
      <div style="display:flex;align-items:center;gap:3px;background:${ac}15;
        ...
        <span contenteditable="true" spellcheck="false"
          style="font-size:calc(0.72rem * var(--rs-font, 1));font-weight:600;color:${ac};outline:none;cursor:text;min-width:20px"
  ...
  const rows = td.rows
    .map((row: string[], ri: number) => {
      ...
      const cells = row
        .map(
          (cell, ci) => `
        <td style="padding:calc(2.5px * var(--rs-gap, 1)) calc(6px * var(--rs-gap, 1));border:1px solid #e2e8f0;font-size:calc(0.76rem * var(--rs-font, 1));
          line-height:calc(1.32 * var(--rs-line, 1));
...
/** Standalone flat bullet-point list section (not nested in an entry) — print-safe */
export function bulletListHtml(sec: AnyData, ac: string): string {
  const n = (sec.bullets || []).length;
  return `<ul style="padding-left:0;margin:0;list-style:none">
    ${(sec.bullets || [])
      .map(
        (b: string, i: number) => `
      <li style="position:relative;display:flex;align-items:flex-start;
          margin-bottom:calc(3px * var(--rs-gap, 1));line-height:calc(1.32 * var(--rs-line, 1))"
        onmouseenter="this.querySelector('.bullet-actions').style.display='flex'"
        onmouseleave="this.querySelector('.bullet-actions').style.display='none'">
        ${bulletDot(ac)}<span contenteditable="true" spellcheck="false"
          style="display:inline;font-size:calc(0.8rem * var(--rs-font, 1));color:#334155;outline:none;border-radius:2px;cursor:text;flex:1;min-width:0"
...
export function dividerHtml(style: string, ac: string): string {
  const gm = 'calc(4px * var(--rs-gap, 1))';
  const styles: Record<string, string> = {
    solid: `border-top:1.5px solid ${ac};margin:${gm} 0`,
    dashed: `border-top:1.5px dashed ${ac}66;margin:${gm} 0`,
    double: `border-top:3px double ${ac};margin:${gm} 0`,
    dotted: `border-top:2.5px dotted ${ac};margin:${gm} 0`,
    thick: `border-top:5px solid ${ac};border-radius:2px;margin:${gm} 0`,
    none: `margin:calc(10px * var(--rs-gap, 1)) 0`,
  };
...
export function renderSection(sec: AnyData, ac: string, compact = false): string {
  switch (sec.type) {
    ...
    default:
      return `<div contenteditable="true" spellcheck="false"
          oninput="typeof handleSectionEntryEdit!=='undefined'&&handleSectionEntryEdit('${sec.id}',this)"
          style="outline:none">${(sec.entries || []).map((en: AnyData) => entryHtml(en, ac, sec.id, compact)).join('')}</div>
        <div contenteditable="false"><button onclick="typeof addEntry!=='undefined'&&addEntry('${sec.id}')"
          class="add-item-btn" style="margin-top:4px">+ Add Entry</button></div>`;
  }
}
...
export function wrapSec(sec: AnyData, ac: string, compact = false): string {
  if (sec.type === 'divider') {
    return `<div data-sec-id="${sec.id}" style="position:relative;margin:calc(6px * var(--rs-gap, 1)) 0">
      ${secCtrl(sec.id)}${renderSection(sec, ac)}
    </div>`;
  }
  return `<div data-sec-id="${sec.id}" style="position:relative;margin-bottom:calc(10px * var(--rs-gap, 1))">
    ${secCtrl(sec.id)}
...
export function sideContent(sec: AnyData, ac: string): string {
  ...
  if (t === 'skills' && sec.skill_groups) {
    return sec.skill_groups
      .map(
        (g: AnyData) => `
      <div style="margin-bottom:calc(7px * var(--rs-gap, 1));overflow:hidden">
        <div style="font-size:calc(0.62rem * var(--rs-font, 1));font-weight:700;margin-bottom:2px;word-break:break-word">${esc(g.category)}</div>
        <div style="font-size:calc(0.62rem * var(--rs-font, 1));opacity:.85;line-height:calc(1.4 * var(--rs-line, 1));word-break:break-word;overflow-wrap:anywhere">${esc((g.skills || []).join(', '))}</div>
      </div>`
      )
      .join('');
  }
  if (t === 'skills-bars' && sec.skills) {
    return sec.skills
      .map(
        (s: AnyData) => `
      <div style="margin-bottom:calc(6px * var(--rs-gap, 1))">
        <div style="font-size:calc(0.62rem * var(--rs-font, 1));font-weight:600;margin-bottom:2px;word-break:break-word">${esc(s.name)}</div>
  ...
  if (t === 'skills-dots' && sec.skills) {
    return sec.skills
      .map(
        (s: AnyData) => `
      <div style="display:flex;justify-content:space-between;margin-bottom:calc(4px * var(--rs-gap, 1));gap:4px">
        <span style="font-size:calc(0.62rem * var(--rs-font, 1));font-weight:500;flex:1;word-break:break-word">${esc(s.name)}</span>
  ...
  if (t === 'skills-tags' && sec.tags) {
    return `<div style="display:flex;flex-wrap:wrap;gap:3px">
      ${sec.tags.map((tag: string) => `<span style="font-size:calc(0.58rem * var(--rs-font, 1));padding:2px 6px;background:rgba(255,255,255,.15);border-radius:100px">${esc(tag)}</span>`).join('')}
    </div>`;
  }
  ...
  return (sec.entries || [])
    .map(
      (en: AnyData) => `
    <div style="margin-bottom:calc(6px * var(--rs-gap, 1));overflow:hidden">
      <div style="font-size:calc(0.62rem * var(--rs-font, 1));font-weight:700;word-break:break-word;line-height:1.3">${esc(en.title)}</div>
      ${en.subtitle ? `<div style="font-size:calc(0.6rem * var(--rs-font, 1));opacity:.8;word-break:break-word">${esc(en.subtitle)}</div>` : ''}
      ${(en.date_start || en.date_end) ? `<div style="font-size:calc(0.59rem * var(--rs-font, 1));opacity:.65">${esc([en.date_start, en.date_end].filter(Boolean).join('–'))}</div>` : ''}
    </div>`
    )
    .join('');
}
