...
export interface EngineCallbacks {
  ...
  onPreviewShow: () => void;
  onDataChange?: (data: AnyData) => void;
}
...
export function initEngine(
  ...
  if (initialData) {
    ...
    const savedTemplate = TEMPLATES.find((tpl) => tpl.id === initialData.layout_config?.template_id);
    if (savedTemplate) (window as any).ACTIVE_TPL = savedTemplate;
    currentFont = typeof initialData.layout_config?.font_id === 'string' ? initialData.layout_config.font_id : 'inter';
    fontScale = typeof initialData.layout_config?.font_scale === 'number' ? initialData.layout_config.font_scale : 1;
    rerenderResume();
    const font = FONTS.find((item) => item.id === currentFont) || FONTS[0];
    contentEl.style.fontFamily = font.css;
    contentEl.style.fontSize = `${fontScale}em`;
    cb.onPreviewShow();
    cb.onActionBtnsShow();
    cb.onHeaderSync();
  }
...
export function rerenderResume() {
  ...
  initDragSections();
  cb?.onDataChange?.(data);
}

/** Updates the document from the controlled builder form, then keeps the
 * existing save/undo engine in sync with the static canvas preview. */
export function replaceResumeData(data: AnyData) {
  if ((window as any).currentData) pushUndo();
...
function attachGlobals() {
  ...
  // One editing host per experience/project entry keeps native keyboard
  // selection and clipboard order together: title → details → bullets for
  // each entry, regardless of how many entries a section contains.
  w.handleEntryEdit = function (secId: string, entryId: string, entryEl: HTMLElement) {
    const selection = window.getSelection();
    const anchor = selection?.anchorNode;
    const anchorEl = anchor?.nodeType === Node.ELEMENT_NODE
      ? (anchor as Element)
      : anchor?.parentElement;
    const fieldEl = anchorEl?.closest?.('[data-edit-field]') as HTMLElement | null;
    if (!fieldEl || !entryEl.contains(fieldEl)) return;

    const field = fieldEl.dataset.editField;
    if (!field) return;
    const bulletIndex = field === 'bullet' ? Number(fieldEl.dataset.bulletIndex) : -1;
    w.handleEdit(field, secId, entryId, Number.isInteger(bulletIndex) ? bulletIndex : -1, fieldEl.textContent || '');
  };

  w.handleSectionEntryEdit = function (secId: string, sectionEl: HTMLElement) {
    const selection = window.getSelection();
    const anchor = selection?.anchorNode;
    const anchorEl = anchor?.nodeType === Node.ELEMENT_NODE
      ? (anchor as Element)
      : anchor?.parentElement;
    const fieldEl = anchorEl?.closest?.('[data-edit-field]') as HTMLElement | null;
    const entryEl = anchorEl?.closest?.('[data-entry-id]') as HTMLElement | null;
    if (!fieldEl || !entryEl || !sectionEl.contains(fieldEl)) return;
...
export const FONTS = [
  { id: 'inter', name: 'Inter', css: "'Inter',Arial,sans-serif", gUrl: 'Inter:wght@400;500;600;700;800' },
  { id: 'lato', name: 'Lato', css: "'Lato',Arial,sans-serif", gUrl: 'Lato:wght@400;700;900' },
  { id: 'source-sans', name: 'Source Sans 3', css: "'Source Sans 3',Arial,sans-serif", gUrl: 'Source+Sans+3:wght@400;500;600;700' },
  { id: 'calibri', name: 'Calibri', css: "Calibri,Arial,sans-serif", gUrl: null as string | null },
...
export function prepareCleanHtml(containerElement: HTMLElement): string {
  ...
  clone
    .querySelectorAll('.sec-drag-handle, .sec-controls, .bullet-actions, .add-item-btn')
    .forEach((el) => el.remove());
  return clone.outerHTML;
}
export async function handleExport(containerElement: HTMLElement): Promise<void> {
