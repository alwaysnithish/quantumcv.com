...
export function entryHtml(en: AnyData, ac: string, secId: string, compact = false): string {
  ...
  const mb = compact ? '5px' : '8px';

  return `<div class="resume-entry" data-entry-id="${en.id}" style="margin-bottom:${mb}">
    <div style="font-size:${fs}rem;font-weight:700;color:#0f172a;word-break:break-word"
    ><span data-edit-field="title"
      style="border-radius:2px;cursor:text"
    >${esc(en.title)}</span><span data-edit-field="date"
      style="font-weight:400;font-size:0.69rem;color:#64748b;font-family:monospace;border-radius:2px;cursor:text"
    >&nbsp;·&nbsp;${esc(dateStr)}</span></div>
    ${
      en.subtitle
        ? `<div data-edit-field="subtitle"
      style="font-size:0.75rem;color:${ac};font-weight:600;margin:1px 0 3px;
        border-radius:2px;word-break:break-word;cursor:text"
    >${esc(en.subtitle)}</div>`
        : ''
    }
    ...
    ${(en.bullets || [])
      .map(
        (b: string, bi: number) => `
      <div class="resume-bullet" style="position:relative;margin-bottom:2px;line-height:1.32;padding-left:13px;text-indent:-13px"
        ...
        <span contenteditable="false" style="color:${ac};font-size:0.68rem">▸</span><span data-edit-field="bullet" data-bullet-index="${bi}"
          style="display:inline;font-size:${bfs}rem;color:#334155;border-radius:2px;
            word-break:break-word;cursor:text;line-height:1.32"
        >${esc(b)}</span>
      ...
      .join('')}
    <div contenteditable="false" style="margin-top:3px">
...
export function skillGroupsHtml(sec: AnyData, ac: string): string {
  return (sec.skill_groups || [])
    ...
    <div style="display:flex;gap:8px;margin-bottom:5px;align-items:flex-start;min-width:0">
      ...
      >${esc(g.category)}</div>
      <div contenteditable="true" spellcheck="false"
        style="font-size:0.73rem;color:#334155;line-height:1.4;flex:1;min-width:0;
...
export function summaryHtml(sec: AnyData): string {
  return `<p contenteditable="true" spellcheck="false"
    style="font-size:0.8rem;color:#334155;line-height:1.4;margin:0;
...
export function langHtml(sec: AnyData): string {
  return (sec.entries || [])
    .map(
      (en: AnyData) => `
    <div style="font-size:0.77rem;margin-bottom:3px">
...
export function bulletListHtml(sec: AnyData, ac: string): string {
  ...
  return `<ul style="padding-left:11px;margin:0">
    ${(sec.bullets || [])
      .map(
        (b: string, i: number) => `
      <li style="position:relative;margin-bottom:3px;line-height:1.32;list-style:none;padding-left:13px;text-indent:-13px"
        onmouseenter="this.querySelector('.bullet-actions').style.display='flex'"
        onmouseleave="this.querySelector('.bullet-actions').style.display='none'">
        <span style="color:${ac};font-size:0.68rem">▸</span><span contenteditable="true" spellcheck="false"
          style="display:inline;font-size:0.8rem;color:#334155;outline:none;border-radius:2px;cursor:text"
...
export function wrapSec(sec: AnyData, ac: string, compact = false): string {
  ...
  return `<div data-sec-id="${sec.id}" style="position:relative;margin-bottom:10px">
    ${secCtrl(sec.id)}
  ...
  if (t === 'skills' && sec.skill_groups) {
    return sec.skill_groups
      ...
      <div style="margin-bottom:7px;overflow:hidden">
        ...
        <div style="font-size:0.62rem;opacity:.85;line-height:1.4;word-break:break-word;overflow-wrap:anywhere">${esc((g.skills || []).join(', '))}</div>
      </div>`
      )
      .join('');
  }
